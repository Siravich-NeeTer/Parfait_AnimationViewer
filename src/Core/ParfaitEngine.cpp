...
void ParfaitEngine::CreateDescriptorSetLayout()
{
...
void ParfaitEngine::CreateColorResource()
{
	VkFormat colorFormat = m_SwapchainImageFormat;

	CreateImage(m_SwapchainExtent.width, m_SwapchainExtent.height, 1, m_MSAA_Samples, colorFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_ColorImage, m_ColorImageMemory);
	m_ColorImageView = CreateImageView(m_ColorImage, colorFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);
}
void ParfaitEngine::CreateDepthResource()
{
	VkFormat depthFormat = FindDepthFormat();

	CreateImage(m_SwapchainExtent.width, m_SwapchainExtent.height, 1, m_MSAA_Samples,depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_DepthImage, m_DepthImageMemory);
	m_DepthImageView = CreateImageView(m_DepthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 1);
}
void ParfaitEngine::CreateTextureImage()
{
...
void ParfaitEngine::LoadModel()
{
	...
	std::unordered_map<Parfait::Vertex, uint32_t> uniqueVertices{};

	for (const auto& shape : shapes)
	{
		for (const auto& index : shape.mesh.indices)
		{
			Parfait::Vertex vertex{};
...
void ParfaitEngine::CreateUniformBuffer()
{
	VkDeviceSize bufferSize = sizeof(Parfait::UniformBufferObject);
...
void ParfaitEngine::CreateDescriptorPool()
{
...
void ParfaitEngine::CreateDescriptorSets()
{
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
	{
		...
		bufferInfo.offset = 0;
		bufferInfo.range = sizeof(Parfait::UniformBufferObject);
...
VkSampleCountFlagBits ParfaitEngine::GetMaxUsableSampleCount()
{
...
bool ParfaitEngine::HasStencilComponent(VkFormat format)
{
...
VkCommandBuffer ParfaitEngine::BeginSingleTimeCommands()
{
	VkCommandBufferAllocateInfo allocInfo{};
	allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
	allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
...
void ParfaitEngine::EndSingleTimeCommands(VkCommandBuffer commandBuffer)
{
...
VkFormat ParfaitEngine::FindSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
{
...
VkFormat ParfaitEngine::FindDepthFormat()
{
...
void ParfaitEngine::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
{
...
VkImageView ParfaitEngine::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels)
{
...
void ParfaitEngine::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels)
{
...
void ParfaitEngine::GenerateMipMaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
{
...
void ParfaitEngine::CopyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
{
	VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
void ParfaitEngine::UpdateUniformBuffer(uint32_t currentImage)
{
	...
	Parfait::UniformBufferObject ubo{};
