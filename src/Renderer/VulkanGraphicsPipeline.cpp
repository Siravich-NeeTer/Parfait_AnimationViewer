...
namespace Parfait
{
	namespace Graphics
	{
		...
		VulkanGraphicsPipeline::VulkanGraphicsPipeline(const VulkanContext& _vulkanContext, const VkRenderPass& _vulkanRenderPass, std::vector<VkDescriptorSetLayout> _descriptorSetLayouts, std::vector<std::filesystem::path> _shaderPaths,
			...
			bool depthTestEnable)
		{
			...
			pushConstantRange.stageFlags = VK_SHADER_STAGE_VERTEX_BIT; // Specify the stage using the push constants
			pushConstantRange.offset = 0; // Start from the beginning
			pushConstantRange.size = _pushConstantSize; // Size of the push constant block
